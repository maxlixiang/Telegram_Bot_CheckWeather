...
class WeatherService:
    ...
    def get_cities_weather(self, cities: list[str], timezone: str) -> list[CityWeatherResult]:
        ...
        for city in cities:
            try:
                ...
                results.append(CityWeatherResult(city=city, error=str(exc)))

        if not results or all(result.error for result in results):
            raise WeatherServiceError("天气服务暂时不可用，请稍后再试。")

        return results

    def get_city_weather(self, city: str, timezone: str) -> CityWeatherResult:
        ...
        return self._build_city_weather(city=city, forecast=forecast)
    ...
    def _get_city_location(self, city: str) -> dict:
        ...
        if not results:
            raise WeatherServiceError("未找到城市位置信息。")
    ...
    def _build_city_weather(self, city: str, forecast: dict) -> CityWeatherResult:
        ...
        for index, date_value in enumerate(dates):
            daily_items.append(
                {
                    "date": date_value,
                    "weather": self.describe_weather_code(codes[index] if index < len(codes) else None),
        ...
        return CityWeatherResult(
            ...
            current={
                ...
                "weather": self.describe_weather_code(current.get("weather_code")),
